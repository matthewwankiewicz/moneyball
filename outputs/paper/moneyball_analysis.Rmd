...
fip <- all_years %>% 
  ggplot(aes(W, FIP, color = as.factor(playoffs), alpha = 0.5)) + 
  geom_point() +
...
This next part of the discussion will focus on each variable's p-value. A p-value is run under a hypothesis which says that a variable has no impact in what it is trying to predict. If the p-value is small, it implies that the variable has a significant impact. In most cases we use a significance level of 0.05 to determine if a variable is significant and that is what I will be using. If the p-value is under 0.05, we can conclude that the variable is significant. In this case, as seen in Table \@ref(tab:coefficients), all of the p-values present are under 0.05 except `before_75` which was just a variable used for classification.
